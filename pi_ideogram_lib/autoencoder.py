"""Flux2 KL autoencoder (official implementation, Apache-2.0).

128-channel packed latent (32 z-channels x 2x2 patch), 8x VAE downscale + 2x patch = 16x total.
Consumes flux2-vae.safetensors directly (already in native KL-ae layout).
"""
...
class AutoEncoder(nn.Module):
  def __init__(self, params: AutoEncoderParams):
    ...
    self.decoder = Decoder(
      resolution=params.resolution, in_channels=params.in_channels, ch=params.ch,
      out_ch=params.out_ch, ch_mult=params.ch_mult, num_res_blocks=params.num_res_blocks,
...
def convert_diffusers_state_dict(src: dict[str, Tensor]) -> dict[str, Tensor]:
  ...
  for src_key, tensor in src.items():
    if src_key.startswith(("encoder.", "decoder.", "bn.", "quant_conv.", "post_quant_conv.")):
      # already in native KL-ae layout (e.g. flux2-vae.safetensors) - passthrough
      out[src_key] = tensor
      continue
...
def _rewrite_diffusers_key(key: str) -> str | None:
  ...
  if m:
    ...
    res_idx, rest = m.group(2), m.group(3).replace("conv_shortcut", "nin_shortcut")
